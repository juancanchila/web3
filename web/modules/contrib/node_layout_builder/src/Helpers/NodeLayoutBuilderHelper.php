...
class NodeLayoutBuilderHelper {
   ...
   *   Element wanted.
   */
  public static function getElementFromArrayData(array &$array, array $pathresult, bool $widthKeys = NULL) {
    $path = [];
    $cur = &$path;
  ...
  public static function getEntityViewModes($entity_type = 'node') {
    $view_modes = [];
    $entity_info = \Drupal::service('entity_display.repository')->getViewModes($entity_type);
  ...
  public static function getAllBlocksByTheme($theme = 'bartik') {
    $blocks = [];
    $blocks_entities = \Drupal::entityTypeManager()
      ->getStorage('block')
    ...
    if ($is_admin) {
      if ($entity instanceof Node) {
        ...
        if ($nlb_manager::nodeLayoutBuilderIsEnabled($entity->getType())) {
          ...
          $response = new RedirectResponse($url->toString() . '?layout-builder=1');
  ...
  public static function getAllViews() {
    ...
    foreach ($views as $view) {
      $list[$view->id()] = $view->get('label');
    }
    return $list;
  }


  public static function loadView($view_id) {
  ...
  public static function renderView($view_id, $display_id, array $arguments) {
    $view = Views::getView($view_id);
    if ($view) {
      $render_array = $view->buildRenderable($display_id, $arguments);
      $rendered = \Drupal::service('renderer')->renderRoot($render_array);
    }
    else {
      $rendered = t("The view ID '$view_id' for display '$display_id' not found");
    }
    return $rendered;
  }

  public static function getViewDisplayIds($view) {
    $options = [];
  ...
  public static function getViewsWithDisplays() {
    $list = [];
    $views = Views::getAllViews();
    foreach ($views as $view) {
      $displays = [];
