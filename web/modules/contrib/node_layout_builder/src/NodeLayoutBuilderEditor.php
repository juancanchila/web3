...
class NodeLayoutBuilderEditor {
  ...
  public static function buttonAddNewSection($nid, $id_element) {
    return [
      '#type' => 'link',
      '#title' => Markup::create('<i class="fas fa-plus"></i>'),
      '#url' => Url::fromRoute(
  ...
  public static function renderBtnActions($type, $nid, $id_element, $parent = 0) {
    ...
    $btns_actions[] = [
      '#type' => '#markup',
      '#markup' => '<span class="btn btn-default icon-move"> <span class="type-name">' . $type . '</span></span>',
    ];
    ...
    $btns_actions[] = [
      '#type' => 'link',
      '#title' => Markup::create('<i class="fas fa-edit"></i>'),
      '#url' => Url::fromRoute(
    ...
    $btns_actions[] = [
      '#type' => 'link',
      '#title' => Markup::create('<i class="fas fa-copy"></i>'),
      '#url' => Url::fromRoute(
    ...
    $btns_actions[] = [
      '#type' => 'link',
      '#title' => Markup::create('<i class="fas fa-trash"></i>'),
      '#url' => Url::fromRoute(
   ...
   */
  public static function getFormByTypeElement(array $element, $type, int $nid = NULL, $class_parent) {
    ...
    switch ($type) {
      ...
      case 'image':
        $form['settings']['image_data']['from'] = [
          ...
          '#default_value' => isset($values['image_data']['from']) ? $values['image_data']['from'] : 1,
          '#attributes' => [
      ...
      case 'node':
        ...
        if ($nid) {
          $node_default = \Drupal::entityTypeManager()
            ->getStorage('node')
            ->load($nid);
        }
        $form['settings']['node'] = [
      ...
      case 'field':
        ...
        $node = Node::load($nid);
        $display = NodeLayoutBuilderHelper::getEntityViewDiplay('node', $node->getType());
      ...
      case 'views':
        $form['settings']['views_container'] = [
          '#type' => 'container',
          '#prefix' => '<div id="edit-output">',
          '#suffix' => '</div>',
        ];

        $views = NodeLayoutBuilderHelper::getAllViews();
        $views = array_reverse($views);
        $views[NULL] = 'None';
        $views = array_reverse($views);
        $form['settings']['views_container']['views'] = [
          '#type' => 'select',
          '#title' => t('Select a view'),
          '#options' => $views,
          '#validated' => TRUE,
          '#default_value' => isset($values['views_container']['views']) ? $values['views_container']['views'] : NULL,
          '#attributes' => [
            'class' => ['form-control'],
          ],
          '#ajax' => [
            'callback' => '::ajaxChangeDisplayViews', // don't forget :: when calling a class method.
            // 'callback' => [$class_parent, 'myAjaxCallback'], //alternative notation
        ...
        if (isset($values['views_container']['view_displays'])) {
          if ($view = NodeLayoutBuilderHelper::loadView($values['views_container']['views'])) {
            $displays = NodeLayoutBuilderHelper::getViewDisplayIds($view);
          }
        }
        else {
          $displays = [];
        }
        $form['settings']['views_container']['view_displays'] = [
          '#type' => 'select',
          '#title' => t('Select display view'),
          '#options' => $displays,
          '#validated' => TRUE,
          '#default_value' => isset($values['views_container']['view_displays']) ? $values['views_container']['view_displays'] : NULL,
          '#attributes' => [
            'class' => ['form-control'],
          ],
          '#states' => [
            'invisible' => [
              ':input[name="configue[settings][views_container][views]"]' => ['value' => ''],
            ],
          ],
        ];
        break;
      ...
      case 'map':
        $form['settings']['map_gps_zoom'] = [
          '#type' => 'number',
          '#title' => t('Zoom'),
          '#default_value' => !empty($values['map_gps_zoom']) ? $values['map_gps_zoom'] : 12,
          '#attributes' => [
            'class' => ['form-control'],
          ],
        ...
        $form['settings']['map_gps_latitude'] = [
          '#type' => 'textfield',
          '#title' => t('Latitude'),
          '#default_value' => !empty($values['map_gps_latitude']) ? $values['map_gps_latitude'] : 30.4198763,
          '#attributes' => [
            'class' => ['form-control'],
          ],
        ...
        $form['settings']['map_gps_longitude'] = [
          '#type' => 'textfield',
          '#title' => t('Longitude'),
          '#default_value' => !empty($values['map_gps_longitude']) ? $values['map_gps_longitude'] : -9.6477853,
          '#attributes' => [
            'class' => ['form-control'],
          ],
          '#prefix' => '<div class="col-md-4">',
  ...
  public static function attributesAndStylesForm(array $values, array $element, string $type) {
    ...
    // Styles / Position.
    $form['styles']['position'] = [
      '#type' => 'details',
      '#title' => t('Position'),
      '#group' => 'styles_tabs',
      '#open' => FALSE,
    ];
    $form['styles']['position']['enable'] = [
      '#title' => t('Enable'),
      '#type' => 'checkbox',
      '#default_value' => isset($element['#styles']['position']['enable']) ? $element['#styles']['position']['enable'] : 0,
      '#description' => t('Use position style.'),
    ];
    $form['styles']['position']['left'] = [
      '#type' => 'textfield',
      '#title' => t('Image style'),
      '#default_value' => isset($element['#styles']['position']['left']) ? $element['#styles']['position']['left'] : 0,
      '#states' => [
        'invisible' => [
          ':input[name="configue[styles][position][enable]"]' => ['checked' => FALSE],
        ],
      ],
    ];
  ...
  public static function linksCategoriesElements($nid, $parent, $id_element, $type) {
    ...
    $categories_elements['layout']['section'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-minus" aria-hidden="true"></span><span class="name">' . t('Section') . '</span>'
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['layout']['row'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-align-justify" aria-hidden="true"></span><span class="name">' . t('Row') . '</span>'
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['layout']['column'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-th-large" aria-hidden="true"></span><span class="name">' . t('Column') . '</span>'
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['text'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-align-left" aria-hidden="true"></span>' . t('Text')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['image'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-image" aria-hidden="true"></span>' . t('Image')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['video'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fab fa-youtube" aria-hidden="true"></span>' . t('video')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['audio'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-music" aria-hidden="true"></span>' . t('audio')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['map'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-map-marker" aria-hidden="true"></span>' . t('Map')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['btn'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-tint" aria-hidden="true"></span>' . t('Button')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['content']['link'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-link" aria-hidden="true"></span>' . t('Link')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['drupal']['block'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-inbox" aria-hidden="true"></span>' . t('block')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['drupal']['node'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-leaf" aria-hidden="true"></span>' . t('node')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['drupal']['field'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-magnet" aria-hidden="true"></span>' . t('Node fields')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
    ...
    $categories_elements['drupal']['views'] = [
      '#type' => 'link',
      '#title' => Markup::create(
        '<span class="fas fa-list-alt" aria-hidden="true"></span>' . t('Views')
      ),
      '#url' => Url::fromRoute(
        'node_layout_builder.element.add',
        [
  ...
  public static function saveElementEntity($nid, $uuid, array $data) {
    ...
    foreach ($data as $key => $element) {
      if (empty($key)) {
        // Remove element that haven't a key.
        unset($data[$key]);
      }
    }
